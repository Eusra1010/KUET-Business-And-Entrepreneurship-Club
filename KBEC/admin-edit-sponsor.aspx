...
<head runat="server">
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', sans-serif; }
        ...
        .current-logo-info { flex: 1; }
        .current-logo-info span { display: block; color: #888; font-size: 0.8rem; }
        .current-logo-info strong { color: #ccc; font-size: 0.88rem; }
...
<script runat="server">
    ...
    void LoadSponsor()
    {
        ...
        {
            conn.Open();
            SqlCommand cmd = new SqlCommand("SELECT * FROM Sponsors WHERE Id=@Id", conn);
            cmd.Parameters.AddWithValue("@Id", Request.QueryString["id"]);
            SqlDataReader reader = cmd.ExecuteReader();
            if (reader.Read())
            {
                txtName.Text = reader["Name"].ToString();
                txtUrl.Text = reader["WebsiteUrl"] == DBNull.Value ? "" : reader["WebsiteUrl"].ToString();
                txtOrder.Text = reader["DisplayOrder"].ToString();
                hdnCurrentLogo.Value = reader["LogoPath"].ToString();
                currentLogoImg.Src = reader["LogoPath"].ToString();
                currentLogoName.InnerText = reader["Name"].ToString();
            }
            else
            {
                Response.Redirect("admin-sponsors.aspx");
    ...
    void btnSave_Click(object sender, EventArgs e)
    {
        ...
        string logoPath = hdnCurrentLogo.Value;
        ...
        using (SqlConnection conn = new SqlConnection(connStr))
        {
            ...
            SqlCommand cmd = new SqlCommand(
                "UPDATE Sponsors SET Name=@Name, LogoPath=@Logo, WebsiteUrl=@Url, DisplayOrder=@Order WHERE Id=@Id", conn);
...
<div class="auth-wrapper">
    <div class="auth-box auth-box-wide">
        ...
        <h1 class="auth-title">Edit Sponsor</h1>
        <p class="auth-subtitle">Update sponsor details or replace the logo</p>
        ...
        <div class="auth-form">
            ...
            <div class="form-group">
                <div class="current-logo-box">
                    <img id="currentLogoImg" runat="server" src="" alt="Current logo" />
                    <div class="current-logo-info">
                        <span>Currently active logo for</span>
                        <strong id="currentLogoName" runat="server"></strong>
                    </div>
                </div>
            </div>

            <div class="section-label" style="margin-top:8px;">Sponsor Details</div>
            <hr class="section-divider" />
            ...
            <div class="section-label" style="margin-top:8px;">Replace Logo</div>
            <hr class="section-divider" />

            <div class="form-group">
                <label>New Logo <span style="color:#555;font-size:0.8rem;">(optional — leave blank to keep current)</span></label>
                <div class="signup-upload-box" id="uploadBox">
                    ...
                    <span class="signup-upload-text">Click to upload new logo</span>
            ...
            <asp:HiddenField ID="hdnCurrentLogo" runat="server" />
...
<script>
    ...
    function submitForm() {
        ...
        if (!name) {
            document.getElementById('nameError').textContent = 'Sponsor name is required.';
            return;
        }
